...
if sys.platform.startswith('win'):
    try:
        print("Applying Windows-specific music fixes...")
        # Try the comprehensive music fix first
        try:
            from fix_music import fix_music_files
            print("Running comprehensive music fix...")
            fix_music_files()
            print("Music fix completed")
            # Try to import the music configuration
            try:
                from music_config import direct_play_music
                print("Successfully loaded music configuration")
                DIRECT_MUSIC_PLAY_AVAILABLE = True
            except ImportError:
                print("Music configuration not available")
                DIRECT_MUSIC_PLAY_AVAILABLE = False
        except ImportError:
            print("Comprehensive music fix not available, trying fallback...")
            from windows_fix import fix_windows_music
            fix_windows_music()
            DIRECT_MUSIC_PLAY_AVAILABLE = False
    except ImportError:
        print("Windows fix modules not found, continuing without fixes")
        DIRECT_MUSIC_PLAY_AVAILABLE = False
    except Exception as e:
        print(f"Error applying Windows fixes: {e}")
        DIRECT_MUSIC_PLAY_AVAILABLE = False
else:
    DIRECT_MUSIC_PLAY_AVAILABLE = False
...
def run_overdrive_menu(screen, font, music_manager=None):
    ...
    vertical_center = SCREEN_HEIGHT // 2 - 30  # Positioned higher on the screen
    button_spacing = 40
    ...
    while True:
        ...
        # Draw gesture control instructions when hovering over gesture button
        if gesture_btn.is_hovered:
            # Create a semi-transparent background for instructions
            instruction_surface = pygame.Surface((300, 200), pygame.SRCALPHA)
            instruction_surface.fill((0, 0, 0, 180))
            instruction_x = SCREEN_WIDTH - 320
            instruction_y = SCREEN_HEIGHT - 220
            ...
            instructions = [
                ...
                "Pinky finger up: Move left",
                "Thumb up: Rotate",
                "Middle finger up: Move down",
                "All fingers up: Hard drop"
            ]
            
            instruction_font = pygame.font.Font(None, 24)
            for i, text in enumerate(instructions):
                color = (255, 100, 100) if i == 0 else (220, 220, 220)
                text_surface = instruction_font.render(text, True, color)
                screen.blit(text_surface, (instruction_x + 10, instruction_y + 10 + i * 30))
...
def run_crazy_mode(screen, font, music_manager=None, use_gestures=False):  # HellF**K Mode
    ...
    if use_gestures:
        # Import the GestureController from gesture_control.py
        try:
            from gesture_control import GestureController
            print("Successfully imported gesture_control module")
            
            # Define controls mapping for gesture to keyboard events
            controls = {
                "left": pygame.K_LEFT,
                "right": pygame.K_RIGHT,
                "down": pygame.K_DOWN,
            ...
            try:
                ...
                show_camera = SETTINGS.get('show_camera_window', True)
                ...
                print("Falling back to keyboard controls due to camera initialization error")
            ...
            gesture_controller = None
            # Fallback to keyboard controls
            print("Falling back to keyboard controls due to import error")
    else:
    ...
    gesture_actions = {"left": False, "right": False, "down": False, "rotate": False, "hard_drop": False}
    last_gesture_action_time = time.time()
    gesture_cooldown = 0.3  # Cooldown between gesture actions to prevent rapid inputs
    ...
    while True:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                if use_gestures and gesture_controller:
            ...
            # Check for direct menu button clicks when game is over
                ...
                if event.key == pygame.K_ESCAPE:
                    ...
                    # Only handle keyboard input if not using gestures
                    if not use_gestures:
                        result = game.handle_input(event)
                        if result == "menu":
                            return "menu"
        ...
        if use_gestures:
            try:
                ...
                if gesture_controller and gesture_controller.camera_available:
                    ...
                    if gesture == "escape":
                        ...
                        # Map gestures to game actions using synthetic keyboard events
                        # This is the same approach used in gesture mode
